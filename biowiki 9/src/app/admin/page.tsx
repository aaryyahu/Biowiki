...
interface ArticleRow   { id: string; status: string }
interface EmbeddingRow { article_id: string }
interface RunRow {
  id: string; status: string; topic: string;
  papers_found: number; findings_extracted: number;
...
async function getAdminStats() {
  ...
  const embeddings  = (embeddingsRes.data ?? []) as EmbeddingRow[]
  const embeddedIds = new Set(embeddings.map((e: EmbeddingRow) => e.article_id))
  const published   = articles.filter((a: ArticleRow) => a.status === 'published')

  return {
    articleTotal:     articles.length,
    articlePublished: published.length,
    articleDraft:     articles.filter((a: ArticleRow) => a.status === 'draft').length,
...
export default async function AdminDashboard() {
  ...
  const statCards: StatCard[] = [
    ...
    { label: 'Articles embedded',  value: stats.embeddedCount,    href: null },
    {
      ...
      value: stats.articlePublished > 0
        ? Math.round((stats.embeddedCount / stats.articlePublished) * 100) + '%'
        : '—',
      href: null,
    },
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(({ label, value, href, accent }: StatCard) => {
          const card = (
        ...
        <div className="card overflow-hidden">
          {stats.recentRuns.length === 0 ? (
            ...
            <div className="divide-y" style={{ '--tw-divide-opacity': 1 } as React.CSSProperties}>
              {stats.recentRuns.map((run: RunRow) => {
                  ...
                  <div key={run.id} className="flex items-center gap-4 px-5 py-3.5"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
                        ...
                        {run.error_message && (
                          <span className="text-red-400 ml-2">{run.error_message.slice(0, 60)}</span>
                        )}
                      </div>
                    </div>
                    {duration !== null && (
